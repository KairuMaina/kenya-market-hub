  ...
  >
    ...
    <CardContent className="p-4 flex-grow">
      ...
      <div className="flex gap-2 mt-2 flex-wrap">
        <Button className="flex-grow min-w-[120px]" size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); alert('Book appointment with ' + provider.full_name); }}>
          Book Appointment
        </Button>
        <Button className="flex-grow min-w-[120px]" size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); alert('Contact ' + provider.full_name); }}>
          Contact
        </Button>
      </div>
    </div>
    ...
    <>
      ...
      {selectedProvider && (
        ...
        >
          ...
          >
            <h2 className="text-2xl font-bold mb-4">{selectedProvider.full_name}</h2>
            <p className="mb-2">
              <strong>Type:</strong> {selectedProvider.provider_type}
            </p>
            <p className="mb-2">
              <strong>Specialization:</strong> {(selectedProvider.specialization as any)?.name || 'General'}
            </p>
            <p className="mb-2">
              <strong>Rating:</strong> {selectedProvider.rating.toFixed(1)}
            </p>
